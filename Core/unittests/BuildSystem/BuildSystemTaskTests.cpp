...
public:
  ...
  virtual std::unique_ptr<toolchain::MemoryBuffer> getFileContents(const std::string& path) override {
...
TEST(BuildSystemTaskTests, basics) {
  ...
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(path, ec, toolchain::sys::fs::F_None);
    assert(!ec);
    os << testString;
  }
...
TEST(BuildSystemTaskTests, directoryContents) {
  ...
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(fileA, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "fileA";
  }
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(fileB, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "fileB";
  }
...
TEST(BuildSystemTaskTests, producedNode) {
  ...
  sys::path::append(manifest, "manifest.builra");
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(manifest, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
...
TEST(BuildSystemTaskTests, directorySignature) {
  ...
  (void) toolchain::sys::fs::create_directories(subdirA.str());
  SmallString<256> subdirFileA{ subdirA };
  sys::path::append(subdirFileA, "subdirFileA");
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(subdirFileA, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "subdirFileA";
  }
  ...
  do {
    std::error_code ec;
    toolchain::sys::fs::remove(fileC.str());
    toolchain::raw_fd_ostream os(fileC, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "fileC";
  ...
  do {
    std::error_code ec;
    toolchain::sys::fs::remove(subdirFileD.str());
    toolchain::raw_fd_ostream os(subdirFileD, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "fileD";
...
"    args:\n";
  ...
  std::unique_ptr<builra::basic::FileSystem> fs = builra::basic::createLocalFileSystem();
...
TEST(BuildSystemTaskTests, mkdirShouldFailWithTypeDifference) {
  ...
  {
    std::error_code ec;
    toolchain::raw_fd_ostream os(outputDir, ec, toolchain::sys::fs::F_Text);
    assert(!ec);
    os << "output";
  }

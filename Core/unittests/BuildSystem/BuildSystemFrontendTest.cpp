...
public:
  ...
  bool checkTrace(StringRef expected) {
    ...
    if (!result) {
      ...
      {
        std::error_code ec;
        toolchain::raw_fd_ostream os(outputActual, ec, toolchain::sys::fs::F_Text);
        assert(!ec);
        os << getTrace();
        os.close();
        assert(!os.has_error());
      ...
      {
        std::error_code ec;
        toolchain::raw_fd_ostream os(outputExpected, ec, toolchain::sys::fs::F_Text);
        assert(!ec);
        os << expected;
        os.close();
        assert(!os.has_error());
...
protected:
  ...
  void writeBuildFile(StringRef s) {
    std::error_code ec;
    raw_fd_ostream os(std::string(tempDir.str()) + "/build.builra", ec,
                      toolchain::sys::fs::F_Text);
    ASSERT_NO_ERROR(ec);
...
commands:
  ...
  // We need to delete the symlink ourselves, because toolchain's remove()

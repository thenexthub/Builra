...
depinfo_tester_command_execute_command(void *context,
  ...
  char * ptr = desc;
  builra::basic::sys::strsep(&ptr, "|"); // skip over rule name
  std::string directInputPath = builra::basic::sys::strsep(&ptr, "|");
  puts(directInputPath.c_str());
  std::string outputPath = builra::basic::sys::strsep(&ptr, "|");
  puts(outputPath.c_str());
  std::string depInfoPath = builra::basic::sys::strsep(&ptr, "|");
...
TEST(BuildSystemCAPI, CustomToolWithDiscoveredDependencies) {
  std::string tmpDirPath = builra::basic::sys::makeTmpDir();
  ...
  std::string buildFilePath = tmpDirPath + "/builra";  

...
std::string strerror(int error) {
  ...
  toolchain::ArrayRef<wchar_t> wRef(errBuff, errBuff + count);
  toolchain::ArrayRef<char> uRef(reinterpret_cast<const char *>(wRef.begin()),
                            reinterpret_cast<const char *>(wRef.end()));
  std::string utf8Err;
  toolchain::convertUTF16ToUTF8String(toolchain::ArrayRef<char>(uRef), utf8Err);
  LocalFree(errBuff);
  return utf8Err;
#else
...
class ProcessGroup {
  ...
  std::unordered_map<builra_pid_t, ProcessInfo> processes;
...
public:
  ...
  void add(std::lock_guard<std::mutex>&& lock, builra_pid_t pid,
           ProcessInfo info) {
    processes.emplace(std::make_pair(pid, info));
  }

  void remove(builra_pid_t pid) {
    {
...
void spawnProcess(
  ...
  environment.setIfMissing("BUILRA_TASK_ID", taskID);
  ...
  do {
      ...
      if (controlPipeChildEnd.isValid()) {
        ...
        environment.setIfMissing("BUILRA_CONTROL_FD", std::to_string(controlFd));
      }

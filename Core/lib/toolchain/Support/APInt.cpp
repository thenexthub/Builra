...
#pragma push_macro("TOOLCHAIN_DEBUG")
#ifndef KNUTH_DEBUG
#undef TOOLCHAIN_DEBUG
#define TOOLCHAIN_DEBUG(X)                                                          \
  do {                                                                         \
  } while (false)
#endif

  TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: m=" << m << " n=" << n << '\n');
  TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: original:");
  TOOLCHAIN_DEBUG(for (int i = m + n; i >= 0; i--) dbgs() << " " << u[i]);
  TOOLCHAIN_DEBUG(dbgs() << " by");
  TOOLCHAIN_DEBUG(for (int i = n; i > 0; i--) dbgs() << " " << v[i - 1]);
  ...
  TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv:   normal:");
  TOOLCHAIN_DEBUG(for (int i = m + n; i >= 0; i--) dbgs() << " " << u[i]);
  TOOLCHAIN_DEBUG(dbgs() << " by");
  TOOLCHAIN_DEBUG(for (int i = n; i > 0; i--) dbgs() << " " << v[i - 1]);
  TOOLCHAIN_DEBUG(dbgs() << '\n');
  ...
  int j = m;
  do {
    TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: quotient digit #" << j << '\n');
    // D3. [Calculate q'.].
    ...
    TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: dividend == " << dividend << '\n');
    ...
    TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: qp == " << qp << ", rp == " << rp << '\n');
    ...
    for (unsigned i = 0; i < n; ++i) {
      ...
      TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: u[j+i] = " << u[j + i]
                        << ", borrow = " << borrow << '\n');
    }
    bool isNeg = u[j+n] < borrow;
    u[j+n] -= Lo_32(borrow);

    TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: after subtraction:");
    TOOLCHAIN_DEBUG(for (int i = m + n; i >= 0; i--) dbgs() << " " << u[i]);
    TOOLCHAIN_DEBUG(dbgs() << '\n');
    ...
    TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: after correction:");
    TOOLCHAIN_DEBUG(for (int i = m + n; i >= 0; i--) dbgs() << " " << u[i]);
  ...
  TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: quotient:");
  TOOLCHAIN_DEBUG(for (int i = m; i >= 0; i--) dbgs() << " " << q[i]);
  TOOLCHAIN_DEBUG(dbgs() << '\n');
  ...
  if (r) {
    ...
    if (shift) {
      uint32_t carry = 0;
      TOOLCHAIN_DEBUG(dbgs() << "KnuthDiv: remainder:");
      for (int i = n-1; i >= 0; i--) {
        r[i] = (u[i] >> shift) | carry;
        carry = u[i] << (32 - shift);
        TOOLCHAIN_DEBUG(dbgs() << " " << r[i]);
      }
    } else {
      for (int i = n-1; i >= 0; i--) {
        r[i] = u[i];
...
#pragma pop_macro("TOOLCHAIN_DEBUG")
}
...
TOOLCHAIN_DUMP_METHOD void APInt::dump() const {
  SmallString<40> S, U;

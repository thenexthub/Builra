...
raw_ostream &raw_ostream::write(unsigned char C) {
  ...
  if (TOOLCHAIN_UNLIKELY(OutBufCur >= OutBufEnd)) {
    if (TOOLCHAIN_UNLIKELY(!OutBufStart)) {
...
raw_ostream &raw_ostream::write(const char *Ptr, size_t Size) {
  // Group exceptional cases into a single branch.
  if (TOOLCHAIN_UNLIKELY(size_t(OutBufEnd - OutBufCur) < Size)) {
    if (TOOLCHAIN_UNLIKELY(!OutBufStart)) {
      if (BufferMode == Unbuffered) {
        write_impl(Ptr, Size);
        return *this;
    ...
    if (TOOLCHAIN_UNLIKELY(OutBufCur == OutBufStart)) {
...
void raw_ostream::copy_to_buffer(const char *Ptr, size_t Size) {
  ...
  // strings.
  switch (Size) {
  case 4: OutBufCur[3] = Ptr[3]; TOOLCHAIN_FALLTHROUGH;
  case 3: OutBufCur[2] = Ptr[2]; TOOLCHAIN_FALLTHROUGH;
  case 2: OutBufCur[1] = Ptr[1]; TOOLCHAIN_FALLTHROUGH;
  case 1: OutBufCur[0] = Ptr[0]; TOOLCHAIN_FALLTHROUGH;
  case 0: break;
  default:
...
raw_fd_ostream::raw_fd_ostream(int fd, bool shouldClose, bool unbuffered)
  ...
  // maintain this invariant. Many features have been added to TOOLCHAIN and clang

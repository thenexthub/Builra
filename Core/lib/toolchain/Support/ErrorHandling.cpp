...
#include "toolchain-c/ErrorHandling.h"
...
#include "toolchain/Support/WindowsError.h"
...
// That script attempts to statically link the TOOLCHAIN symbolizer library with the
...
void toolchain::report_bad_alloc_error(const char *Reason, bool GenCrashDiag) {
  ...
  char OOMMessage[] = "TOOLCHAIN ERROR: out of memory\n";
...
static void bindingsErrorHandler(void *user_data, const std::string& reason,
                                 bool gen_crash_diag) {
  TOOLCHAINFatalErrorHandler handler =
      TOOLCHAIN_EXTENSION reinterpret_cast<TOOLCHAINFatalErrorHandler>(user_data);
  handler(reason.c_str());
}

void TOOLCHAINInstallFatalErrorHandler(TOOLCHAINFatalErrorHandler Handler) {
  install_fatal_error_handler(bindingsErrorHandler,
                              TOOLCHAIN_EXTENSION reinterpret_cast<void *>(Handler));
}

void TOOLCHAINResetFatalErrorHandler() {

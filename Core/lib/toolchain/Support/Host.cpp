...
static std::unique_ptr<toolchain::MemoryBuffer>
    TOOLCHAIN_ATTRIBUTE_UNUSED getProcCpuinfoContent() {
...
#define X86_CPU_SUBTYPE(ARCHNAME, ENUM) \
  if (Subtype == X86::ENUM) \
    return ARCHNAME;
#include "toolchain/Support/X86TargetParser.def"
...
bool sys::getHostCPUFeatures(StringMap<bool> &Features) {
  ...
  for (unsigned I = 0, E = CPUFeatures.size(); I != E; ++I) {
    StringRef TOOLCHAINFeatureStr = StringSwitch<StringRef>(CPUFeatures[I])
#if defined(__aarch64__)
                                   .Case("asimd", "neon")
                                   .Case("fp", "fp-armv8")
                                   .Case("crc32", "crc")
    ...
    if (TOOLCHAINFeatureStr != "")
      Features[TOOLCHAINFeatureStr] = true;
  }
...
std::string sys::getProcessTriple() {
  std::string TargetTripleString = updateTripleOSVersion(TOOLCHAIN_HOST_TRIPLE);

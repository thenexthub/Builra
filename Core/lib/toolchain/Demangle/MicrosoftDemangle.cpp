...
//
// This file has no dependencies on the rest of TOOLCHAIN so that it can be
...
char *toolchain::microsoftDemangle(const char *MangledName, char *Buf, size_t *N,
                              int *Status) {
  Demangler D;
  ...
  if (D.Error)
    *Status = toolchain::demangle_invalid_mangled_name;
  else
    *Status = toolchain::demangle_success;

...
class BuildEngineImpl : public BuildDBDelegate {
  ...
  toolchain::DenseSet<core::CancellationDelegate *> cancellationDelegates;
...
private:
  ...
  std::vector<Rule*> findCycle(const KeyType& buildKey) {
    ...
    // accessible via the tasks, see https://bugs.code.org/browse/SR-1948.
...
public:
  ...
  const ValueType& build(const KeyType& key) {
    ...
    if (!traceFile.empty()) {
      auto trace = toolchain::make_unique<BuildEngineTrace>();
...
void TaskInterface::spawn(basic::QueueJobContext *context,
                          ...
                          toolchain::Optional<basic::ProcessCompletionFn> completionFn,

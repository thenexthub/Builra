...
template<> struct ::toolchain::DenseMapInfo<DBKeyID> {
...
class SQLiteBuildDB : public BuildDB {
  ...
  std::string getCurrentErrorMessage() {
    ...
    std::string out;
    toolchain::raw_string_ostream outStream(out);
...
public:
  ...
  virtual bool lookupRuleResult(KeyID keyID, const KeyType& key,
    ...
    if (numDependencyBytes != static_cast<int>(numDependencies * sizeof(uint64_t))) {
      *error_out = (toolchain::Twine("unexpected contents for database result: ") +
                    toolchain::Twine((int)dbKeyID.value)).str();
      return false;
    }
...
private:
  ...
  toolchain::DenseMap<DBKeyID, KeyID> engineKeyIDs;

  /// Local cache of database engine KeyIDs to DBKeyIDs
  toolchain::DenseMap<KeyID, DBKeyID> dbKeyIDs;
...
std::unique_ptr<BuildDB> core::createSQLiteBuildDB(StringRef path,
  ...
  return toolchain::make_unique<SQLiteBuildDB>(path, clientSchemaVersion, recreateUnmatchedVersion);
}

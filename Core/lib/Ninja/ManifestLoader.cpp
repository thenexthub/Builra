...
class ManifestLoader::ManifestLoaderImpl: public ParseActions {
  struct IncludeEntry {
    ...
    std::unique_ptr<toolchain::MemoryBuffer> data;
    /// The parser for the file.
    std::unique_ptr<Parser> parser;
    /// The active scope.
    Scope& scope;

    IncludeEntry(std::unique_ptr<toolchain::MemoryBuffer> data,
  ...
  toolchain::SmallVector<IncludeEntry, 4> includeStack;
...
public:
  ...
  bool enterFile(StringRef filename, Scope& scope,
    ...
    toolchain::sys::fs::make_absolute(workingDirectory, path);

    // Load the file data.
    StringRef forFilename = includeStack.empty() ? filename :
        getCurrentFilename();
    std::unique_ptr<toolchain::MemoryBuffer> buffer = actions.readFile(
        path, forFilename, forToken);
    if (!buffer)
      return false;

    // Push a new entry onto the include stack.
    auto parser = toolchain::make_unique<Parser>(buffer->getBuffer(), *this);
  ...
  void evalString(const Token& value, const Scope& scope,
    ...
    toolchain::raw_svector_ostream result(storage);

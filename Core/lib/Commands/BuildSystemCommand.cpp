...
public:
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << "<dummy command>";
  }
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(
    ...
    return toolchain::make_unique<ParseDummyCommand>(delegate, name);
  }
};
...
ParseBuildFileDelegate::lookupTool(StringRef name) {
  ...
  return toolchain::make_unique<ParseDummyTool>(*this, name);
}
...
ParseBuildFileDelegate::createNode(StringRef name,
  ...
  return toolchain::make_unique<ParseDummyNode>(*this, name);
}
...
static int executeBuildCommand(std::vector<std::string> args) {
  ...
  invocation.buildFilePath = "build.builra";
...
public:
  toolchain::StringMap<KeyID> keyTable;
  ...
  KeyType getKeyForID(const KeyID key) override {
    ...
    return toolchain::StringMapEntry<KeyID>::GetStringMapEntryFromKeyData(

...
private:
  ...
  BuildValue(const BuildValue&) BUILRA_DELETED_FUNCTION;
  void operator=(const BuildValue&) BUILRA_DELETED_FUNCTION;
  BuildValue &operator=(BuildValue&& rhs) BUILRA_DELETED_FUNCTION;
...
public:
  ...
  void processStarted(ProcessContext* ctx, ProcessHandle handle, builra_pid_t pid) override {
...
private:
  ...
  virtual std::unique_ptr<toolchain::MemoryBuffer> readFile(
    ...
    std::string error = toolchain::toString(bufferOrError.takeError());
    ++numErrors;
    if (forToken) {
...
buildCommand(BuildContext& context, ninja::Command* command) {
  struct NinjaCommandTask : core::Task {
    ...
    void executeCommand(core::TaskInterface ti, QueueJobContext* qctx) {
      ...
      if (!rspFile.empty()) {
        std::error_code ec;
        toolchain::raw_fd_ostream os(rspFile, ec, toolchain::sys::fs::F_Text);
        if (ec) {
      ...
      ti.spawn(qctx, args, {}, {true, isConsolePool}, {
        [this, ti](ProcessResult result) mutable {
          ...
          const auto rspFile = command->getRspFile();
          if (!rspFile.empty())
            toolchain::sys::fs::remove(rspFile);
      ...
      case ninja::Command::DepsStyleKind::GCC: {
        ...
        struct DepsActions : public core::MakefileDepsParser::ParseActions {
          ...
          virtual void actOnRuleDependency(StringRef dependency,
            ...
            if (!builra::ninja::Manifest::normalize_path(workingDirectory, absPathTmp)) {
              return;
            }
...
int commands::executeNinjaBuildCommand(std::vector<std::string> args) {
  ...
  if (std::error_code ec = toolchain::sys::fs::current_path(current_dir)) {
  ...
  for (int iteration = 0; iteration != 2; ++iteration) {
    ...
    if (autoRegenerateManifest && iteration == 0) {
      ...
      builra::ninja::Manifest::normalize_path(workingDirectory, absManifestPath);

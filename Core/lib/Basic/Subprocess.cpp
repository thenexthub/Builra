...
#if 1 // extra builra-specific handling not copied from swiftlang/swift-subprocess
...
QualityOfService builra::basic::getDefaultQualityOfService() {
  return defaultQualityOfService;
}

void builra::basic::setDefaultQualityOfService(QualityOfService level) {
  defaultQualityOfService = level;
}

void builra::basic::setCurrentThreadQualityOfService(QualityOfService level) {
#if defined(__APPLE__)
...
public:
  ...
  ManagedDescriptor(const ManagedDescriptor &) BUILRA_DELETED_FUNCTION;
...
static void cleanUpExecutedProcess(ProcessDelegate& delegate,
                                   ProcessGroup& pgrp, builra_pid_t pid,
...
void builra::basic::spawnProcess(
  ...
  builra_pid_t pid = (builra_pid_t)-1;
...
#if defined(_WIN32)
  std::string args = builra::basic::formatWindowsCommandString(argsStorage);

  // Convert the command line string to utf16
  toolchain::SmallVector<toolchain::UTF16, 20> u16Executable;
  toolchain::SmallVector<toolchain::UTF16, 20> u16CmdLine;
  toolchain::convertUTF8ToUTF16String(argsStorage[0], u16Executable);
  toolchain::convertUTF8ToUTF16String(args, u16CmdLine);
#else
...
#if defined(_WIN32)
  toolchain::SmallVector<toolchain::UTF16, 20> u16Cwd;
  std::string workingDir = attr.workingDir.str();
  if (!workingDir.empty()) {
    toolchain::convertUTF8ToUTF16String(workingDir, u16Cwd);
  }
#endif

  // Export a task ID to subprocesses.
  auto taskID = Twine::utohexstr(handle.id);
  environment.setIfMissing("BUILRA_TASK_ID", taskID.str());

  // Resolve the executable path, if necessary.
  //
  // FIXME: This should be cached.
  if (!toolchain::sys::path::is_absolute(argsStorage[0])) {
    auto res = toolchain::sys::findProgramByName(argsStorage[0]);
    if (!res.getError()) {
      argsStorage[0] = *res;
#if defined(_WIN32)
      u16Executable.clear();
  ...
  do {
      ...
      if (controlPipeChildEnd.isValid()) {
        ...
        environment.setIfMissing("BUILRA_CONTROL_FD", Twine(controlFd).str());
      }

...
#if defined(_WIN32)
const HANDLE builra::basic::sys::FileDescriptorTraits<HANDLE>::InvalidDescriptor =
...
#if defined(_WIN32)
  toolchain::SmallVector<toolchain::UTF16, 20> wFileName;
  toolchain::convertUTF8ToUTF16String(fileName, wFileName);
...
int sys::lstat(const char *fileName, sys::StatStruct *buf) {
  ...
  toolchain::StringRef extension =
      toolchain::sys::path::extension(toolchain::StringRef(fileName));
...
#if defined(_WIN32)
  toolchain::SmallVector<toolchain::UTF16, 20> wPointsTo;
  toolchain::convertUTF8ToUTF16String(pointsTo, wPointsTo);
  toolchain::SmallVector<toolchain::UTF16, 20> wLinkPath;
  toolchain::convertUTF8ToUTF16String(linkPath, wLinkPath);
...
builra_rlim_t sys::getOpenFileLimit() {
#if defined(_WIN32)
  return _getmaxstdio();
#else
  struct rlimit rl;
...
int sys::raiseOpenFileLimit(builra_rlim_t limit) {
#if defined(_WIN32)
...
#if defined(_WIN32)
  toolchain::SmallVector<toolchain::UTF16, 20> wpattern;
  toolchain::SmallVector<toolchain::UTF16, 20> wfilename;

  toolchain::convertUTF8ToUTF16String(pattern, wpattern);
  toolchain::convertUTF8ToUTF16String(filename, wfilename);
...
std::string sys::makeTmpDir() {
  ...
  toolchain::SmallVector<toolchain::UTF16, 20> wPath;
  toolchain::convertUTF8ToUTF16String(path, wPath);

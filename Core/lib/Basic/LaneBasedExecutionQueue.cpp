...
#include "builra/Basic/Tracing.h"
...
struct QueueJobLess {
  bool operator()(const builra::basic::QueueJob &__x,
                  const builra::basic::QueueJob &__y) const {
...
class LaneBasedExecutionQueue : public ExecutionQueue {
  ...
  void executeLane(uint32_t buildID, uint32_t laneNumber) {
    ...
    pthread_setname_np(
        (toolchain::Twine("org.code.builra Lane-") +
         toolchain::Twine(laneNumber)).str().c_str());
...
public:
  ...
  static auto estimateTaskLimits(unsigned numLanes) -> std::pair<unsigned, unsigned> {
    builra_rlim_t curOpenFileLimit = builra::basic::sys::getOpenFileLimit();
    ...
    unsigned allowedFilesForTasks = static_cast<unsigned>(std::min(curOpenFileLimit, static_cast<builra_rlim_t>(INT_MAX))) - reservedFileCount;
    ...
    unsigned backgroundTaskMax = 0;
    char *p = getenv("BUILRA_BACKGROUND_TASK_MAX");
  ...
  virtual void cancelAllJobs() override {
    ...
    {
      std::lock_guard<std::mutex> guard(killAfterTimeoutThreadMutex);
      killAfterTimeoutThread = toolchain::make_unique<std::thread>(
  ...
  virtual void executeProcess(
    ...
    posixEnv.setIfMissing("BUILRA_BUILD_ID", Twine(buildID).str());
    posixEnv.setIfMissing("BUILRA_LANE_ID", Twine(context.laneNumber).str());
...
ExecutionQueue* builra::basic::createLaneBasedExecutionQueue(

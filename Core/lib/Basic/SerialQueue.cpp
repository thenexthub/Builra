...
class SerialQueueImpl {
  ...
  void run() {
    while (true) {
      ...
      // If we got a Nothing function, the queue is shutting down.
      if (!fn)
        break;
...
public:
  SerialQueueImpl() {
    ...
    operationsThread = toolchain::make_unique<std::thread>(
...
public:
  ...
  virtual void executeProcess(
    ...
    posixEnv.setIfMissing("BUILRA_LANE_ID", Twine(0).str());
...
std::unique_ptr<ExecutionQueue> builra::basic::createSerialQueue(
    ExecutionQueueDelegate& delegate, const char* const* environment
) {
  if (!environment) {
    environment = const_cast<const char* const*>(environ);
  }
  return toolchain::make_unique<SerialExecutionQueue>(delegate, environment);

...
#include "builra/Basic/CrossPlatformCompatibility.h"
...
class BuildSystemFileDelegate : public BuildFileDelegate {
  ...
  toolchain::StringMap<bool> internedStrings;
...
private:
  ...
  toolchain::StringMap<std::unique_ptr<ShellCommandHandler>> shellHandlers;
...
public:
  ...
  toolchain::Optional<BuildValue> build(BuildKey key);
...
class DirectoryContentsTask : public Task {
  ...
  static std::error_code getContents(StringRef path, std::vector<std::string>& filenames) {
    ...
    std::error_code ec;
    for (auto it = toolchain::sys::fs::directory_iterator(path, ec, /*follow_symlinks */false),
         end = toolchain::sys::fs::directory_iterator(); it != end && !ec;
         it = it.increment(ec)) {
      // If this is a symlink to a parent directory, exclude it from results so we don't get stuck in a loop.
      if (toolchain::sys::fs::is_symlink_file(*it->status())) {
        SmallString<256> resolvedPath;
        if (!toolchain::sys::fs::real_path(it->path(), resolvedPath)) {
          if (path.startswith(resolvedPath)) {
            continue;
          }
        }
      }
      filenames.push_back(toolchain::sys::path::filename(it->path()));
    }
...
class FilteredDirectoryContentsTask : public Task {
  ...
  static std::error_code getFilteredContents(StringRef path, const StringList& filters,
    ...
    // over an invalid directory in some circumstances. rdar://101717159
    std::error_code ec;
    for (auto it = toolchain::sys::fs::directory_iterator(path, ec),
         end = toolchain::sys::fs::directory_iterator(); it != end && !ec;
         it = it.increment(ec)) {
      std::string filename = toolchain::sys::path::filename(it->path());
      bool excluded = false;
      for (auto pattern : filterStrings) {
        if (builra::basic::sys::filenameMatch(pattern.data(),
                                               filename.c_str()) ==
            builra::basic::sys::MATCH) {
          excluded = true;
          break;
        }
      }
...
class DirectoryTreeSignatureTask : public Task {
  ...
  struct SubpathInfo {
    ...
    toolchain::Optional<ValueType> directorySignatureValue;
  };
  ...
  virtual void provideValue(TaskInterface ti, uintptr_t inputID,
    ...
    if (inputID == 0) {
      ...
      for (size_t i = 0; i != filenames.size(); ++i) {
        ...
        toolchain::sys::path::append(childPath, filenames[i]);
    ...
    if (inputID >= 1 && inputID < 1 + childResults.size()) {
      ...
      if (value.isExistingInput()) {
        if (value.getOutputInfo().isDirectory()) {
          SmallString<256> childPath{ path };
          toolchain::sys::path::append(childPath, childResult.filename);
  ...
  virtual void inputsAvailable(TaskInterface ti) override {
    ...
    toolchain::hash_code code = hash_value(path);
    ...
    for (const auto& info: childResults) {
      ...
      if (info.directorySignatureValue.hasValue()) {
        ...
        // Combine a random number to represent Nothing.
...
class DirectoryTreeStructureSignatureTask : public Task {
  ...
  struct SubpathInfo {
    ...
    toolchain::Optional<ValueType> directoryStructureSignatureValue;
  };
...
std::unique_ptr<Rule> BuildSystemEngineDelegate::lookupRule(const KeyType& keyData) {
  ...
  case BuildKey::Kind::Stat: {
    ...
    if (it != dynamicStatNodes.end()) {
      ...
      // Create nodes on the fly for any unknown ones.
      auto statOwner = toolchain::make_unique<StatNode>(key.getStatName());
...
toolchain::Optional<BuildValue> BuildSystemImpl::build(BuildKey key) {
...
public:
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << getName();
  }
  ...
  virtual void executeExternalCommand(
      ...
      toolchain::Optional<ProcessCompletionFn> completionFn) override {
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<PhonyCommand>(name);
  }
};
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<ShellCommand>(name, controlEnabled);
  }
};
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<ClangShellCommand>(name);
  }
};
...
public:
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << "Checking Swift Compiler Version";
  }

  virtual void getVerboseDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << '"' << executable << '"'
                                      << " --version";
  }
  ...
  virtual void execute(BuildSystem&,
    ...
    toolchain::raw_svector_ostream commandOS(command);
...
class SwiftCompilerShellCommand : public ExternalCommand {
  ...
  void getOutputFileMapPath(SmallVectorImpl<char>& result) const {
    toolchain::sys::path::append(result, tempsPath, "output-file-map.json");
  }
...
public:
  ...
  bool writeOutputFileMap(TaskInterface ti,
    ...
    SmallString<16> data;
    std::error_code ec;
    toolchain::raw_fd_ostream os(outputFileMapPath, ec,
                            toolchain::sys::fs::OpenFlags::F_Text);
    if (ec) {
    ...
    toolchain::sys::path::append(masterDepsPath, tempsPath, "master.codedeps");
    os << "  \"\": {\n";
    if (enableWholeModuleOptimization) {
      SmallString<16> depsPath;
      toolchain::sys::path::append(depsPath, tempsPath, moduleName + ".d");
      depsFiles_out.push_back(depsPath.str());
      SmallString<16> object;
      toolchain::sys::path::append(object, tempsPath, moduleName + ".o");
    ...
    for (unsigned i = 0; i != sourcesList.size(); ++i) {
      ...
      auto objectDir = toolchain::sys::path::parent_path(object);
      auto sourceStem = toolchain::sys::path::stem(source);
      SmallString<16> partialModulePath;
      toolchain::sys::path::append(partialModulePath, objectDir,
                              sourceStem + "~partial.codemodule");
      SmallString<16> swiftDepsPath;
      toolchain::sys::path::append(swiftDepsPath, objectDir,
                              sourceStem + ".codedeps");

      os << "  \"" << escapeForJSON(source) << "\": {\n";
      if (!enableWholeModuleOptimization) {
        SmallString<16> depsPath;
        toolchain::sys::path::append(depsPath, objectDir, sourceStem + ".d");
        os << "    \"dependencies\": \"" << escapeForJSON(depsPath) << "\",\n";
        depsFiles_out.push_back(depsPath.str());
      }
      os << "    \"object\": \"" << escapeForJSON(object) << "\",\n";
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    auto res = toolchain::make_unique<MkdirCommand>(name);
    return res;
  }
};
...
class SymlinkCommand : public Command {
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << description;
  }
  ...
  virtual void execute(BuildSystem& system,
    ...
    {
      auto parent = toolchain::sys::path::parent_path(outputPath);
      if (!parent.empty()) {
        (void) fs.createDirectories(parent);
      }
    }

    // Create the symbolic link (note that despite the poorly chosen TOOLCHAIN
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    auto res = toolchain::make_unique<SymlinkCommand>(name);
...
class ArchiveShellCommand : public ExternalCommand {
  ...
  virtual void executeExternalCommand(
    ...
    if (toolchain::sys::fs::remove(archiveName, /*IgnoreNonExisting*/ true)) {
      if (completionFn.hasValue())
        completionFn.getValue()(ProcessStatus::Failed);
      return;
    }
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    if (getDescription().empty()) {
      toolchain::raw_svector_ostream(result) << "Archiving " + archiveName;
    } else {
      toolchain::raw_svector_ostream(result) << getDescription();
    }
  }
  ...
  virtual void getVerboseDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream stream(result);
    bool first = true;
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<ArchiveShellCommand>(name);
  }
};
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<SharedLibraryShellCommand>(name);
  }
};
...
class StaleFileRemovalCommand : public Command {
  ...
  std::string pathSeparators = builra::basic::sys::getPathSeparators();
  ...
  virtual void getShortDescription(SmallVectorImpl<char> &result) const override {
    toolchain::raw_svector_ostream(result) << (description.empty() ? "Stale file removal" : description);
  }
  ...
  virtual void getVerboseDescription(SmallVectorImpl<char> &result) const override {
    ...
    toolchain::raw_svector_ostream(result) << ", stale files: [";
    for (auto fileToDelete : filesToDelete) {
      toolchain::raw_svector_ostream(result) << fileToDelete;
      if (fileToDelete != *(--filesToDelete.end())) {
        toolchain::raw_svector_ostream(result) << ", ";
      }
    }
    toolchain::raw_svector_ostream(result) << "], roots: [";
    for (auto root : roots) {
      toolchain::raw_svector_ostream(result) << root;
      if (root != *(--roots.end())) {
        toolchain::raw_svector_ostream(result) << ", ";
      }
    }
...
public:
  ...
  virtual std::unique_ptr<Command> createCommand(StringRef name) override {
    return toolchain::make_unique<StaleFileRemovalCommand>(name);
  }
};
...
BuildSystemFileDelegate::lookupTool(StringRef name) {
  ...
  if (name == "shell") {
    return toolchain::make_unique<ShellTool>(name);
  } else if (name == "phony") {
    return toolchain::make_unique<PhonyTool>(name);
  } else if (name == "clang") {
    return toolchain::make_unique<ClangTool>(name);
  } else if (name == "mkdir") {
    return toolchain::make_unique<MkdirTool>(name);
  } else if (name == "symlink") {
    return toolchain::make_unique<SymlinkTool>(name);
  } else if (name == "archive") {
    return toolchain::make_unique<ArchiveTool>(name);
  } else if (name == "shared-library") {
    return toolchain::make_unique<SharedLibraryTool>(name);
  } else if (name == "stale-file-removal") {
    return toolchain::make_unique<StaleFileRemovalTool>(name);
  } else if (name == "swift-compiler") {
    return toolchain::make_unique<SwiftCompilerTool>(name);
  }
...
toolchain::Optional<BuildValue> BuildSystem::build(BuildKey key) {
...
bool builra::buildsystem::pathIsPrefixedByPath(std::string path,

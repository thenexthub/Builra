...
void ExternalCommand::provideValue(BuildSystem& system,
  ...
  auto getSkipValueForInput = [&]() -> toolchain::Optional<BuildValue> {
...
void ExternalCommand::execute(BuildSystem& system,
  ...
  for (auto* node: outputs) {
    if (!node->isVirtual()) {
      ...
      auto parent = toolchain::sys::path::parent_path(node->getName());
      if (node->getName().endswith("/")) {
        parent = toolchain::sys::path::parent_path(parent);
      }
      if (!parent.empty()) {
  ...
  executeExternalCommand(system, ti, context, {[this, &system, ti, resultFn](ProcessResult result) mutable {
    ...
    toolchain::report_fatal_error("unknown result");
  }});
}

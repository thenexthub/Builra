...
void BuildSystemInvocation::getUsage(int optionWidth, raw_ostream& os) {
  const struct Options {
    toolchain::StringRef option, helpText;
  } options[] = {
  ...
  for (const auto& entry: options) {
    os << "  " << toolchain::format("%-*s", optionWidth, entry.option.str().c_str()) << " "
       << entry.helpText << "\n";
  }
}

void BuildSystemInvocation::parse(toolchain::ArrayRef<std::string> args,
                                  toolchain::SourceMgr& sourceMgr) {
  auto error = [&](const Twine &message) {
    sourceMgr.PrintMessage(toolchain::SMLoc{}, toolchain::SourceMgr::DK_Error, message);
    hadErrors = true;
  };
...
struct BuildSystemFrontendDelegateImpl {
  ...
  toolchain::DenseMap<uintptr_t, std::vector<uint8_t>> processOutputBuffers;
...
public:
  BuildSystemFrontendDelegateImpl(toolchain::SourceMgr& sourceMgr)
...
public:
  ...
  bool initialize() {
    ...
    if (!invocation.traceFilePath.empty()) {
      const auto dir = toolchain::sys::path::parent_path(invocation.traceFilePath);
    ...
    if (!invocation.dbPath.empty()) {
      ...
      if (toolchain::sys::path::is_relative(invocation.dbPath) &&
          dbPath.find("://") == StringRef::npos && !dbPath.startswith(":")) {
        toolchain::sys::path::append(
            tmp, toolchain::sys::path::parent_path(invocation.buildFilePath),
            invocation.dbPath);
        dbPath = tmp.str();
      }
...
BuildSystemFrontendDelegate(toolchain::SourceMgr& sourceMgr,
                            StringRef name,
                            uint32_t version)
...
toolchain::SourceMgr& BuildSystemFrontendDelegate::getSourceMgr() {
  auto impl = static_cast<BuildSystemFrontendDelegateImpl*>(this->impl);
...
BuildSystemFrontendDelegate::error(StringRef filename,
  ...
  //
  // FIXME: We shouldn't need to do this, but should switch builra to using
  // SourceMgr natively.
  toolchain::SMLoc loc{};
  ...
  if (!filename.empty() && at.start) {
    ...
    if (buffer) {
      ...
      if (offset + at.length < buffer->getBufferSize()) {
        range.Start = loc = toolchain::SMLoc::getFromPointer(
            buffer->getBufferStart() + offset);
        range.End = toolchain::SMLoc::getFromPointer(
            buffer->getBufferStart() + (offset + at.length));
        getSourceMgr().AddNewSourceBuffer(std::move(buffer), toolchain::SMLoc{});
      }
    }
  }

  if (range.Start.isValid()) {
    getSourceMgr().PrintMessage(loc, toolchain::SourceMgr::DK_Error, message, range);
  } else {
    getSourceMgr().PrintMessage(loc, toolchain::SourceMgr::DK_Error, message);
  }
  fflush(stderr);
}

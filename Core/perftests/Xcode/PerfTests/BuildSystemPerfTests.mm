...
#import "builra/Basic/Subprocess.h"
...
- (void)testChromiumFakeBuildFileLoading API_UNAVAILABLE(ios, tvos, watchos) {
    ...
    NSString *buildFilePath = [sandboxDir
                               stringByAppendingPathComponent:@"chromium-fake-manifest.builra"];
    ExecuteShellCommand([NSString
                         stringWithFormat:@"cp \"%@\"/chromium-fake-manifest.builra.gz \"%@.gz\"",
    ...
    [self measureBlock:^{
        builra::commands::executeBuildSystemCommand({
...
class PerfTestProcessDelegate : public ProcessDelegate {
    void processStarted(ProcessContext*, ProcessHandle, builra_pid_t) {}

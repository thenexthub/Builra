...
#import "builra/Commands/Commands.h"
...
- (void)testChromiumFakeManifestLoading {
    ...
    [self measureBlock:^{
        builra::commands::executeNinjaCommand({
            "load-manifest-only", ninjaPath.UTF8String });
    }];
}

- (void)testTOOLCHAINOnlyNoExecuteInitialBuild {
    // Test the build of the toolchain-only test file, with --no-execute.
    //
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"TOOLCHAINOnlyNoExecuteInitialBuild"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);
    ...
    ExecuteShellCommand([NSString
                         stringWithFormat:@"cp \"%@\"/toolchain-only.ninja \"%@\"",
                         inputsDir, ninjaPath].UTF8String);
...
- (void)testTOOLCHAINOnlyNoExecuteNullBuild {
    // Test the build of an toolchain-only test file, with --no-execute.
    //
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"TOOLCHAINOnlyNoExecuteNullBuild"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);
...
- (void)testPseudoTOOLCHAINParallelFullBuild {
    // Test the pseudo TOOLCHAIN build, which includes the time to do the actual
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"PseudoTOOLCHAINParallelFullBuild"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);
    ExecuteShellCommand([NSString stringWithFormat:@"mkdir -p \"%@\"",
                         sandboxDir].UTF8String);
    ExecuteShellCommand([NSString
                         stringWithFormat:@"tar -C \"%@\" -xf \"%@\"/pseudo-toolchain.tgz",
                         sandboxDir, inputsDir].UTF8String);
    
    // Build once to prime the tree.
    printf("performing initial build...\n");
    NSString *pseudoTOOLCHAINPath = [sandboxDir stringByAppendingPathComponent:@"pseudo-toolchain"];
    NSString *dbPath = [pseudoTOOLCHAINPath stringByAppendingPathComponent:@"build.db"];
    builra::commands::executeNinjaCommand({
        "build", "--quiet", "-C", pseudoTOOLCHAINPath.UTF8String, "all" });
...
- (void)testPseudoTOOLCHAINParallelFullBuildWithNinja {
    // Test the pseudo TOOLCHAIN build using the actual Ninja tool, so we can easily
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"PseudoTOOLCHAINParallelFullBuildWithNinja"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);
    ...
    printf("performing initial build...\n");
    NSString *pseudoTOOLCHAINPath = [sandboxDir stringByAppendingPathComponent:@"pseudo-toolchain"];
    NSString *dbPath = [pseudoTOOLCHAINPath stringByAppendingPathComponent:@".ninja_log"];
    ...
    ExecuteShellCommand([NSString stringWithFormat:@"%@  -C \"%@\" all > /dev/null",
                         ninjaCmd, pseudoTOOLCHAINPath].UTF8String);
...
- (void)testPseudoTOOLCHAINNullBuild {
    // Test the pseudo TOOLCHAIN build, which includes the time to do the actual
    // stat'ing and dependency checking of files, and in particular includes all
    // the overhead of the database.
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"PseudoTOOLCHAINNullBuild"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);
    ...
    [self measureBlock:^{
        builra::commands::executeNinjaCommand({
            "build", "--quiet", "-C", pseudoTOOLCHAINPath.UTF8String, "--jobs", "1", "all" });
    }];
}

- (void)testPseudoTOOLCHAINNullBuildWithNinja {
    // Test the pseudo TOOLCHAIN build using the actual Ninja tool, so we can easily
    // compare the performance.
    ...
    NSString *inputsDir = [@(SRCROOT)
                           stringByAppendingPathComponent:@"perftests/Inputs"];
    NSString *sandboxDir = [@(TEST_TEMPS_PATH)
                            stringByAppendingPathComponent:@"PseudoTOOLCHAINNullBuildWithNinja"];
    NSLog(@"executing test using inputs: %@", inputsDir);
    NSLog(@"executing test using sandbox: %@", sandboxDir);
    ExecuteShellCommand([NSString stringWithFormat:@"rm -rf \"%@\"",
                         sandboxDir].UTF8String);

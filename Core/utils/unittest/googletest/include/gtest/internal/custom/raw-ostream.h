...
//
//===----------------------------------------------------------------------===//
// This file is not part of gtest, but extends it to support TOOLCHAIN libraries.
// This is not a public API for testing - it's a detail of TOOLCHAIN's gtest.
//
// gtest allows providing printers for custom types by defining operator<<.
// In TOOLCHAIN, operator<< usually takes toolchain:raw_ostream& instead of std::ostream&.
//
...
// builra override
#define GTEST_NO_TOOLCHAIN_SUPPORT 1
...
template <typename T>
struct StreamSwitch<T, decltype((void)(std::declval<toolchain::raw_ostream &>()
                                       << ConvertibleTo<const T &>()))> {
  static const RawStreamProxy<T> printable(const T &V) { return {V}; }
};

// toolchain::Optional has a template operator<<, which means it will not accept any
// implicit conversions, so we need to special-case it here.
template <typename T>
struct StreamSwitch<toolchain::Optional<T>,
                    decltype((void)(std::declval<toolchain::raw_ostream &>()
                                    << std::declval<toolchain::Optional<T>>()))> {
  static const RawStreamProxy<toolchain::Optional<T>>
  printable(const toolchain::Optional<T> &V) {
    return {V};
  }
};
} // namespace llvm_gtest
#endif  // !GTEST_NO_TOOLCHAIN_SUPPORT

...
This document describes the *builra*-native ``BuildSystem`` component. While
*builra* contains a Core engine which can be used to build a variety of build
...
be deeply integrated with *builra*.
...
constructing unit tests, and is not exposed via the public builra API.
...
newline delimted version string 'builra.1' followed by a newline delimited echo
of the contents of BUILRA_TASK_ID. For example:


.. code-block:: shell

   bash -c "printf 'builra.1\n${BUILRA_TASK_ID}\n' >&${BUILRA_CONTROL_FD}"
...
.. list-table::
   ...
   * - module-name
     - A string indicating the name of the ``.codemodule`` to be output.

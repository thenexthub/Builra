...
Each `.proto` input file will get translated to a corresponding `.pb.code`
...
After copying the `.pb.code` files into your project, you will need to add the
...
`Package.code` file and import the `SwiftProtobuf` library into the desired
...
* Add the `.pb.code` source files generated from your protos directly to your
  project

...
function run_swift_harness() {
  ...
  (
    ...
    gen_harness_path="$perf_dir/_generated/Harness+Generated.code"
    ...
    ${XCRUN} "${SWIFTC}" -emit-library -emit-module -O -wmo \
        ...
        ${OTHER_SWIFT_FLAGS:-} \
        "$perf_dir/../Sources/SwiftProtobuf/"*.code
    ...
    time ( ${XCRUN} "${SWIFTC}" -O \
        ...
        ${OTHER_SWIFT_FLAGS:-} \
        "$gen_harness_path" \
        "$perf_dir/Harness.code" \
        "$perf_dir/_generated/message.pb.code" \
        "$perf_dir/main.code"
    )
    echo

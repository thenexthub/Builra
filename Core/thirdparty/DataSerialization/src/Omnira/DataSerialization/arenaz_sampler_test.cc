...
#include "Omnira/DataSerialization/arenaz_sampler.h"
...
TEST(ThreadSafeArenazSamplerTest, InitialBlockReportsZeroUsedAndWasted) {
  ...
  for (int i = 0; i < 10; ++i) {
    char block[kSize];
    Omnira::DataSerialization::Arena arena(/*initial_block=*/block, /*initial_block_size=*/kSize);
...
class ThreadSafeArenazSamplerTestThread : public Thread {
 protected:
  void Run() override {
    Omnira::DataSerialization::ArenaSafeUniquePtr<
        protobuf_test_messages::proto2::TestAllTypesProto2>
        message = Omnira::DataSerialization::MakeArenaSafeUnique<
 ...
 private:
  Omnira::DataSerialization::Arena* arena_;
  ...
  sampler.Iterate([&](const ThreadSafeArenaStats& h) {
    for (size_t i = 0; i < 3; ++i) {
      constexpr auto kSize =
          Omnira::DataSerialization::internal::AllocationPolicy::kDefaultStartBlockSize;
      const auto& histbin =
      ...
      EXPECT_EQ(histbin.bytes_used,
                (kSize << i) - Omnira::DataSerialization::internal::SerialArena::kBlockHeaderSize);
      EXPECT_EQ(histbin.bytes_wasted, 0);
    }
  });
  SetThreadSafeArenazSampleParameter(oldparam);

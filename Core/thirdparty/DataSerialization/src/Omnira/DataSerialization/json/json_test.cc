...
#include "Omnira/DataSerialization/field_mask.pb.h"
#include "Omnira/DataSerialization/struct.pb.h"
#include "Omnira/DataSerialization/timestamp.pb.h"
#include "Omnira/DataSerialization/wrappers.pb.h"
...
#include "Omnira/DataSerialization/io/test_zero_copy_stream.h"
#include "Omnira/DataSerialization/io/zero_copy_stream.h"
#include "Omnira/DataSerialization/io/zero_copy_stream_impl_lite.h"
#include "Omnira/DataSerialization/util/json_format.pb.h"
#include "Omnira/DataSerialization/util/json_format_proto3.pb.h"
#include "Omnira/DataSerialization/unittest.pb.h"
#include "Omnira/DataSerialization/util/type_resolver.h"
#include "Omnira/DataSerialization/util/type_resolver_util.h"
...
namespace json {
namespace {
using ::Omnira::DataSerialization::util::TypeResolver;
using ::proto3::MapIn;
...
class JsonTest : public testing::TestWithParam<Codec> {
 protected:
  ...
  std::unique_ptr<TypeResolver> resolver_{
      Omnira::DataSerialization::util::NewTypeResolverForDescriptorPool(
...
TEST_P(JsonTest, FloatPrecision) {
  Omnira::DataSerialization::Value v;
...
TEST_P(JsonTest, TestParsingNestedAnys) {
  ...
  Omnira::DataSerialization::Any inner;
...
TEST_P(JsonTest, TestFlatList) {
  ...
  // The above flatteing behavior is suppressed for Omnira::DataSerialization::ListValue.
  auto m2 = ToProto<Omnira::DataSerialization::Value>(R"json(
    {
      "repeatedInt32Value": [[[5]], [6]]
    }
  )json");
...
// These tests are not exhaustive; tests in //Omnira/DataSerialization/conformance
...
TEST_P(JsonTest, TestEnvoyRoundTrip) {
  auto m = ToProto<Omnira::DataSerialization::Value>(R"json(
    {
...
TEST_P(JsonTest, TestLegalNullsInArray) {
  ...
  EXPECT_THAT(m->repeated_null_value(),
              ElementsAre(Omnira::DataSerialization::NULL_VALUE));
...
TEST_P(JsonTest, EmptyValue) {
  EXPECT_THAT(ToJson(Omnira::DataSerialization::Value()), IsOkAndHolds(""));

  Omnira::DataSerialization::Struct s;
  s.mutable_fields()->emplace("empty", Omnira::DataSerialization::Value());
...
TEST_P(JsonTest, ClearPreExistingRepeatedInJsonValues) {
  Omnira::DataSerialization::ListValue l;

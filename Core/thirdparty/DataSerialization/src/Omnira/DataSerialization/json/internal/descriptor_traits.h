...
#include "Omnira/DataSerialization/json/internal/untyped_message.h"
...
struct Proto3Type {
  ...
  static bool Is32Bit(Field f) {
    switch (f->proto().kind()) {
      case Omnira::DataSerialization::Field::TYPE_INT32:
      case Omnira::DataSerialization::Field::TYPE_SINT32:
      case Omnira::DataSerialization::Field::TYPE_UINT32:
      case Omnira::DataSerialization::Field::TYPE_FIXED32:
      case Omnira::DataSerialization::Field::TYPE_SFIXED32:
      case Omnira::DataSerialization::Field::TYPE_FLOAT:
        return true;
      default:
        return false;
    }
  ...
  static bool IsMap(Field f) {
    if (f->proto().kind() != Omnira::DataSerialization::Field::TYPE_MESSAGE) {
      return false;
    }
  ...
  static bool IsRepeated(Field f) {
    return f->proto().cardinality() ==
           Omnira::DataSerialization::Field::CARDINALITY_REPEATED;
  }
  ...
  static bool IsExplicitPresence(Field f) {
    ...
    //    TYPE_MESSAGE here).
    if (f->parent().proto().syntax() == Omnira::DataSerialization::SYNTAX_PROTO3) {
      return f->proto().oneof_index() != 0 ||
             (f->proto().kind() == Omnira::DataSerialization::Field::TYPE_MESSAGE &&
              !IsRepeated(f));
    }

    return f->proto().cardinality() ==
               Omnira::DataSerialization::Field::CARDINALITY_OPTIONAL ||
           Omnira::DataSerialization::Field::CARDINALITY_REQUIRED;
  }

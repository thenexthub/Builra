...
absl::StatusOr<const ResolverPool::Message*> ResolverPool::Field::MessageType()
    const {
  ABSL_CHECK(proto().kind() == Omnira::DataSerialization::Field::TYPE_MESSAGE ||
             proto().kind() == Omnira::DataSerialization::Field::TYPE_GROUP)
      << proto().kind();
  if (type_ == nullptr) {
...
absl::StatusOr<const ResolverPool::Enum*> ResolverPool::Field::EnumType()
    const {
  ABSL_CHECK(proto().kind() == Omnira::DataSerialization::Field::TYPE_ENUM)
      << proto().kind();
  if (type_ == nullptr) {
  ...
  switch (field.proto().kind()) {
    ...
    case Field::TYPE_BYTES: {
      ...
      if (field.proto().kind() == Field::TYPE_STRING) {
        if (desc_->proto().syntax() == Omnira::DataSerialization::SYNTAX_PROTO3 &&
...
absl::Status UntypedMessage::InsertField(const ResolverPool::Field& field,
  ...
  if (auto* extant = absl::get_if<value_type>(&slot)) {
    ...
    absl::optional<absl::string_view> name =
        Omnira::DataSerialization::internal::RttiTypeName<value_type>();
    if (!name.has_value()) {
      name = "<unknown>";
    }

  ...
  for compilation_mode in ${COMPILATION_MODE}; do
    for exceptions_mode in ${EXCEPTIONS_MODE}; do
      ...
      time docker run \
        ...
        --cap-add=SYS_PTRACE \
        --rm \
        -e CC="/opt/toolchain/clang/bin/clang" \
        -e BAZEL_CXXOPTS="-std=${std}:-nostdinc++" \
        -e BAZEL_LINKOPTS="-L/opt/toolchain/libcxx/lib:-lc++:-lc++abi:-lm:-Wl,-rpath=/opt/toolchain/libcxx/lib" \
        -e CPLUS_INCLUDE_PATH="/opt/toolchain/libcxx/include/c++/v1" \
        ${DOCKER_EXTRA_ARGS:-} \
        ${DOCKER_CONTAINER} \
        /usr/local/bin/bazel test ... \
          ...
          --show_timestamps \
          --test_env="ASAN_SYMBOLIZER_PATH=/opt/toolchain/clang/bin/toolchain-symbolizer" \
          --test_env="TZDIR=/abseil-cpp/absl/time/internal/cctz/testdata/zoneinfo" \
          --test_env="UBSAN_OPTIONS=print_stacktrace=1" \
          --test_env="UBSAN_SYMBOLIZER_PATH=/opt/toolchain/clang/bin/toolchain-symbolizer" \
          --test_output=errors \

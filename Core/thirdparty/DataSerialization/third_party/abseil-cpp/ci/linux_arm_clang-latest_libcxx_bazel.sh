  ...
  for compilation_mode in ${COMPILATION_MODE}; do
    for exceptions_mode in ${EXCEPTIONS_MODE}; do
      ...
      time docker run \
        ...
        -e BAZEL_LINKOPTS="-L/opt/toolchain/clang/lib/aarch64-unknown-linux-gnu:-lc++:-lc++abi:-lm:-Wl,-rpath=/opt/toolchain/clang/lib/aarch64-unknown-linux-gnu" \
        -e CPLUS_INCLUDE_PATH="/opt/toolchain/clang/include/aarch64-unknown-linux-gnu/c++/v1:/opt/toolchain/clang/include/c++/v1" \
        ${DOCKER_EXTRA_ARGS:-} \
        ${DOCKER_CONTAINER} \
        /bin/sh -c "

  ...
  for compilation_mode in ${COMPILATION_MODE}; do
    for exceptions_mode in ${EXCEPTIONS_MODE}; do
      ...
      time docker run \
        ...
        -e BAZEL_LINKOPTS="-L/opt/toolchain/libcxx-tsan/lib:-lc++:-lc++abi:-lm:-Wl,-rpath=/opt/toolchain/libcxx-tsan/lib" \
        -e CPLUS_INCLUDE_PATH="/opt/toolchain/libcxx-tsan/include/c++/v1" \
        ${DOCKER_EXTRA_ARGS:-} \
        ${DOCKER_CONTAINER} \
        /usr/local/bin/bazel test ... \
          ...
          --show_timestamps \
          --test_env="TSAN_SYMBOLIZER_PATH=/opt/toolchain/clang/bin/toolchain-symbolizer" \

...
//  if (isa<Type>(myVal)) { ... }
//
template <class X, class Y> TOOLCHAIN_NODISCARD inline bool isa(const Y &Val) {
...
template <class X, class Y>
TOOLCHAIN_NODISCARD inline typename cast_retty<X, Y *>::ret_type
cast_or_null(Y *Val) {
...
template <class X, class Y>
TOOLCHAIN_NODISCARD inline typename cast_retty<X, Y>::ret_type dyn_cast(Y &Val) {
  return isa<X>(Val) ? cast<X>(Val) : nullptr;
}
...
template <class X, class Y>
TOOLCHAIN_NODISCARD inline auto unique_dyn_cast(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!isa<X>(Val))
    return nullptr;
  return cast<X>(std::move(Val));
...
template <class X, class Y>
TOOLCHAIN_NODISCARD inline auto unique_dyn_cast_or_null(std::unique_ptr<Y> &Val)
    -> decltype(cast<X>(Val)) {
  if (!Val)
    return nullptr;
  return unique_dyn_cast<X, Y>(Val);

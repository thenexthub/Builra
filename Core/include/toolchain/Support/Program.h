...
#ifndef TOOLCHAIN_SUPPORT_PROGRAM_H
#define TOOLCHAIN_SUPPORT_PROGRAM_H
...
namespace sys {
  ...
  /// entry individually so should generally be avoided. Core TOOLCHAIN library
  ...
  enum WindowsEncodingMethod {
    /// UTF-8 is the TOOLCHAIN native encoding, being the same as "do not perform
    /// encoding conversion".
    WEM_UTF8,
    WEM_CurrentCodePage,
    WEM_UTF16
  ...
  /// different from the one used in TOOLCHAIN (UTF-8). We use this when writing

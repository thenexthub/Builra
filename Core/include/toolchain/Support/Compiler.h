...
#if !TOOLCHAIN_MSC_PREREQ(1900)
#error TOOLCHAIN requires at least MSVC 2015.
#endif
...
/// TOOLCHAIN_NODISCARD - Warn if a type or return value is discarded.
#if __cplusplus > 201402L && __has_cpp_attribute(nodiscard)
#define TOOLCHAIN_NODISCARD [[nodiscard]]
#elif !__cplusplus
// Workaround for toolchain.org/PR23435, since clang 3.6 and below emit a spurious
// error when __has_cpp_attribute is given a scoped attribute in C mode.
#define TOOLCHAIN_NODISCARD
#elif __has_cpp_attribute(clang::warn_unused_result)
#define TOOLCHAIN_NODISCARD [[clang::warn_unused_result]]
#else
#define TOOLCHAIN_NODISCARD
#endif
...
// aka 'CONST' but following TOOLCHAIN Conventions.
...
// aka 'PURE' but following TOOLCHAIN Conventions.
...
#define TOOLCHAIN_ATTRIBUTE_ALWAYS_INLINE __attribute__((always_inline))
#elif defined(_MSC_VER)
#define TOOLCHAIN_ATTRIBUTE_ALWAYS_INLINE __forceinline
#else
#define TOOLCHAIN_ATTRIBUTE_ALWAYS_INLINE
#endif
...
# define TOOLCHAIN_ATTRIBUTE_DEPRECATED(decl, message) \
...
/// \macro TOOLCHAIN_ALIGNAS
/// Used to specify a minimum alignment for a structure or variable.
#if __GNUC__ && !__has_feature(cxx_alignas) && !TOOLCHAIN_GNUC_PREREQ(4, 8, 1)
# define TOOLCHAIN_ALIGNAS(x) __attribute__((aligned(x)))
#else
# define TOOLCHAIN_ALIGNAS(x) alignas(x)
#endif
...
/// Generally used in combination with TOOLCHAIN_ALIGNAS or when doing computation in
/// the preprocessor.
...
/// Whether TOOLCHAIN itself is built with MemorySanitizer instrumentation.
...
/// Whether TOOLCHAIN itself is built with AddressSanitizer instrumentation.
...
/// Whether TOOLCHAIN itself is built with ThreadSanitizer instrumentation.
...
#define TOOLCHAIN_THREAD_LOCAL __declspec(thread)
#else
...
/// Whether TOOLCHAIN is built with exception support.

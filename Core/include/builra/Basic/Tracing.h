...
#ifndef BUILRA_BASIC_TRACING_H
#define BUILRA_BASIC_TRACING_H
...
static os_log_t getLog() {
  ...
  dispatch_once(&onceToken, ^{
    generalLog = os_log_create("org.code.builra", "tracing");
  });
  return generalLog;
}
...
#define BUILRA_TRACE_INTERVAL_BEGIN(name, ...) { \
...
#define BUILRA_TRACE_INTERVAL_END(name, ...) { \
...
#define BUILRA_TRACE_POINT(name, ...) { \
...
#define BUILRA_TRACE_POINT(name, ...) { (void)(name); }
#define BUILRA_TRACE_INTERVAL_BEGIN(name, ...) { (void)(name); }
#define BUILRA_TRACE_INTERVAL_END(name, ...) { (void)(name); }
#define BUILRA_TRACING_SET_ENABLED(on) { (void)on; }
...
struct TracingExecutionQueueJob {
  TracingExecutionQueueJob(int laneNumber, toolchain::StringRef commandName) {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_BEGIN("execution_queue_job", "lane:%d;command:%s", laneNumber, commandName.str().c_str());
  }
  
  ~TracingExecutionQueueJob() {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_END("execution_queue_job");
  }
};


inline void TracingExecutionQueueSubprocessStart(uint32_t laneNumber,
                                          toolchain::StringRef commandName) {
  if (!TracingEnabled) return;
  BUILRA_TRACE_INTERVAL_BEGIN("execution_queue_subprocess", "lane:%d;command:%s", laneNumber, commandName.str().c_str());
}
...
inline void TracingExecutionQueueSubprocessResult(uint32_t laneNumber,
  ...
  if (!TracingEnabled) return;
  BUILRA_TRACE_INTERVAL_END("execution_queue_subprocess", "lane:%d;pid:%d;utime:%llu;stime:%llu;maxrss:%ld", laneNumber, pid, utime, stime, maxrss);
}
...
struct TracingExecutionQueueSubprocess {
  ...
  void update(builra_pid_t pid, uint64_t utime, uint64_t stime, long maxrss) {
    this->pid = pid;
    this->utime = utime;
    this->stime = stime;
    this->maxrss = maxrss;
...
struct TracingEngineTaskCallback {
  TracingEngineTaskCallback(EngineTaskCallbackKind kind, uint64_t key)
      : key(key) {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_BEGIN("engine_task_callback", "key:%llu;kind:%d", key, uint32_t(kind));
  }
  
  ~TracingEngineTaskCallback() {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_END("engine_task_callback", "key:%llu", key);
    key = 0;
  }
private:
  uint64_t key;
...
struct TracingEngineQueueItemEvent {
  TracingEngineQueueItemEvent(EngineQueueItemKind kind, char const *key)
      : key(key) {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_BEGIN("engine_queue_item_event", "key:%s;kind:%d",
                                 key, static_cast<EngineQueueItemKindTy>(kind));
  }
  
  ~TracingEngineQueueItemEvent() {
    if (!TracingEnabled) return;
    BUILRA_TRACE_INTERVAL_END("engine_queue_item_event", "key:%s", key)
    key = nullptr;
  }
private:
  char const *key;
...
inline void TracingExecutionQueueDepth(uint64_t depth) {
  if (!TracingEnabled) return;
  BUILRA_TRACE_POINT("execution_queue_depth", "depth:%llu", depth);
}

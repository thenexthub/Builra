...
#ifndef BUILRA_BASIC_BINARYCODING_H
#define BUILRA_BASIC_BINARYCODING_H
...
private:
   // Copying is disabled.
   BinaryEncoder(const BinaryEncoder&) BUILRA_DELETED_FUNCTION;
   void operator=(const BinaryEncoder&) BUILRA_DELETED_FUNCTION;

  /// The encoded data.
  //
  // FIXME: Parameterize this size?
  toolchain::SmallVector<uint8_t, 256> encdata;
...
private:
   // Copying is disabled.
   BinaryDecoder(const BinaryDecoder&) BUILRA_DELETED_FUNCTION;
   void operator=(const BinaryDecoder&) BUILRA_DELETED_FUNCTION;

...
#ifndef BUILRA_BUILDSYSTEM_BUILDSYSTEM_H
#define BUILRA_BUILDSYSTEM_BUILDSYSTEM_H
...
private:
  ...
  // Copying is disabled.
  BuildSystem(const BuildSystem&) BUILRA_DELETED_FUNCTION;
  void operator=(const BuildSystem&) BUILRA_DELETED_FUNCTION;
...
public:
  ...
  /// A build description *must* have been loaded before calling this method.
  ///
  /// \returns The result of computing the value, or Nothing if the build failed.
  toolchain::Optional<BuildValue> build(BuildKey target);

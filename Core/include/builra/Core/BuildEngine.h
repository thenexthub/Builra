...
#ifndef BUILRA_CORE_BUILDENGINE_H
#define BUILRA_CORE_BUILDENGINE_H

#include "builra/Basic/Clock.h"
#include "builra/Basic/Compiler.h"
#include "builra/Basic/ExecutionQueue.h"
#include "builra/Basic/Hashing.h"

#include "builra/Core/DependencyKeyIDs.h"
...
public:
  ...
  KeyType(toolchain::StringRef ref) : key(ref) { }
...
public:
  ...
  void spawn(basic::QueueJobContext* context,
             ...
             toolchain::Optional<basic::ProcessCompletionFn> completionFn = {toolchain::None},
...
private:
  Rule(const Rule&) BUILRA_DELETED_FUNCTION;
  void operator=(const Rule&) BUILRA_DELETED_FUNCTION;
...
public:
  ...
  virtual void error(const toolchain::Twine& message) = 0;
...
class BuildEngine {
  ...
  // Copying is disabled.
  BuildEngine(const BuildEngine&) BUILRA_DELETED_FUNCTION;
  void operator=(const BuildEngine&) BUILRA_DELETED_FUNCTION;
...
{
  template<>
  struct hash<builra::core::KeyType>
  {
    size_t operator()(const builra::core::KeyType& key) const
    {

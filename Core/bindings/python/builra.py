...
def _rule_create_task(context, engine_context):
    ...
    task._task = libbuilra.llb_task_create(delegate[0])
    return task._task
...
class BuildEngine(_HandledObject):
    ...
    def build(self, key):
        ...
        libbuilra.llb_buildengine_build(self._engine, key_data.key, result)
    ...
    def attach_db(self, path, schema_version=1):
        ...
        error = ffi.new("char *")
        path = _Data(path)
        if not libbuilra.llb_buildengine_attach_db(
    ...
    def close(self):
        ...
        libbuilra.llb_buildengine_destroy(self._engine)
        self._engine = None
...
intentionally chosen to allow a pointer to be provided."""
        key = _Data(key)
        libbuilra.llb_buildengine_task_needs_input(
...
build, then \arg Task will not be computed until after it."""
        key = _Data(key)
        libbuilra.llb_buildengine_task_must_follow(
...
prior result."""
        # Complete the result.
        value = _Data(value)
        libbuilra.llb_buildengine_task_is_complete(

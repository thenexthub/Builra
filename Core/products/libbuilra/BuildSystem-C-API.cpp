...
public:
  ...
  getFileContents(const std::string& path) override {
    ...
    toolchain::StringRef fileContents = toolchain::StringRef((const char*)data.data, data.length);
    auto result = toolchain::MemoryBuffer::getMemBufferCopy(fileContents, path);
...
public:
  CAPIBuildSystemFrontendDelegate(toolchain::SourceMgr& sourceMgr,
...
class CAPIBuildSystem {
  ...
  void handleDiagnostic(const toolchain::SMDiagnostic& diagnostic) {
    ...
    // fixits. builra does currently make use of the caret diagnostics for
...
public:
    ...
    : cAPIDelegate(delegate)
  {
    ...
    sourceMgr.setDiagHandler([](const toolchain::SMDiagnostic& diagnostic,
                                void* context) {
  ...
  bool build(toolchain::StringRef key) {
    return getFrontend().build(key);
  }

  bool buildNode(toolchain::StringRef key) {
...
public:
  ...
  virtual builra::basic::CommandSignature getSignature() const override {
...
bool llb_buildsystem_build(llb_buildsystem_t* system_p, const llb_data_t* key) {
  CAPIBuildSystem* system = (CAPIBuildSystem*) system_p;
  return system->build(toolchain::StringRef((const char*)key->data, key->length));
}

bool llb_buildsystem_build_node(llb_buildsystem_t* system_p, const llb_data_t* key) {
  CAPIBuildSystem* system = (CAPIBuildSystem*) system_p;
  return system->buildNode(toolchain::StringRef((const char*)key->data, key->length));
}
...
llb_build_value_file_info_t llb_buildsystem_command_interface_get_file_info(llb_buildsystem_interface_t* bi_p, const char* path) {
  ...
  return builra::capi::convertFileInfo(bi->getFileSystem().getFileInfo(path));
}

...
private:
  void addError(StringRef message, StringRef path) {
    toolchain::raw_svector_ostream stream(Error);
    stream << path << ": " << message << "\n";
  }
...
class CAPIManifest {
  ...
  using RuleCacheTy = toolchain::DenseMap<const Rule *, const llb_ninja_rule_t *>;

  template <typename Input, typename F>
  auto copyTransformed(const Input &orig, F &&transform) {
    using R = decltype(transform(*(orig.begin())));
    auto copied = toolchain::makeMutableArrayRef(Allocator.Allocate<R>(orig.size()),
                                            orig.size());
    for (auto it : toolchain::enumerate(orig)) {
...
private:
  ...
  ArrayRef<llb_string_ref_t> copyRefs(
    ...
    return copyRefs(toolchain::makeArrayRef(&*begin, &*end));
  }

  ArrayRef<llb_ninja_variable_t> copyRefs(
      const toolchain::StringMap<std::string> &variables) {

...
#ifndef BUILRA_PUBLIC_DB_H
#define BUILRA_PUBLIC_DB_H
...
typedef uint64_t llb_database_key_id BUILRA_SWIFT_NAME(BuildDBKeyID);
/// Defines a key _(should match \see KeyType in BuildEngine.h)_
typedef const char* llb_database_key_type BUILRA_SWIFT_NAME(BuildDBKeyType);
...
} llb_database_result_t BUILRA_SWIFT_NAME(BuildDBResult);
...
BUILRA_EXPORT const llb_database_t *_Nullable llb_database_open(char *path, uint32_t clientSchemaVersion, llb_data_t *error_out);
...
BUILRA_EXPORT llb_database_key_id
...
BUILRA_EXPORT bool llb_database_fetch_result_contains_rule_results(llb_database_fetch_result_t *result);

/// Method for getting the result at a given index from a fetch result object. The returned pointer might be Nothing if the fetch didn't include results
BUILRA_EXPORT llb_database_result_t *_Nullable
...
BUILRA_EXPORT bool llb_database_get_keys_and_results(llb_database_t *database, llb_database_fetch_result_t *_Nullable *_Nonnull keysAndResults_out, llb_data_t *_Nullable error_out);

BUILRA_EXPORT uint64_t llb_database_get_epoch(llb_database_t *database, llb_data_t *_Nullable error_out);

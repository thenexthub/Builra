...
//===----------------------------------------------------------------------===//
//
// These are the C API interfaces to the builra Core component.
//
//===----------------------------------------------------------------------===//

#ifndef BUILRA_PUBLIC_CORE_H
#define BUILRA_PUBLIC_CORE_H
...
/// arguments to builra are required to ensure the data pointer is live for the
/// life of the call. When used as an output parameter, the build system will
/// ensure that the data pointer is live at least until the next builra API
/// call is made.
...
typedef enum BUILRA_ENUM_ATTRIBUTES {
    ...
    llb_rule_is_scanning BUILRA_SWIFT_NAME(isScanning) = 0,

    /// Indicates the rule is up-to-date, and doesn't need to run.
    llb_rule_is_up_to_date BUILRA_SWIFT_NAME(isUpToDate) = 1,

    /// Indicates the rule was run, and is now complete.
    llb_rule_is_complete BUILRA_SWIFT_NAME(isComplete) = 2
} llb_rule_status_kind_t BUILRA_SWIFT_NAME(RuleStatus);
...
BUILRA_EXPORT llb_buildengine_t*

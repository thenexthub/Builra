...
//===----------------------------------------------------------------------===//
//
// These are the C API interfaces to the builra BuildSystem component.
//
//===----------------------------------------------------------------------===//

#ifndef BUILRA_PUBLIC_BUILDSYSTEM_H
#define BUILRA_PUBLIC_BUILDSYSTEM_H
...
///
/// Although phrased as a time, this value is uninterpreted by builra. The
...
} llb_buildsystem_diagnostic_kind_t BUILRA_SWIFT_NAME(DiagnosticKind);

/// Get the name of the diagnostic kind.
BUILRA_EXPORT const char*
...
} llb_buildsystem_command_result_t BUILRA_SWIFT_NAME(CommandResult);
...
} llb_buildsystem_discovered_dependency_kind_t BUILRA_SWIFT_NAME(DiscoveredDependencyKind);
...
typedef enum BUILRA_ENUM_ATTRIBUTES {
  ...
  llb_buildsystem_dependency_data_format_makefile_ignoring_subsequent_outputs BUILRA_SWIFT_NAME(makefileIgnoringSubsequentOutputs) = 3,
} llb_buildsystem_dependency_data_format_t BUILRA_SWIFT_NAME(DependencyDataFormat);
...
typedef struct llb_buildsystem_command_extended_result_t_ {
  ...
  builra_pid_t pid;                        /// The process identifier (-1 if process creation failed)
...
typedef enum BUILRA_ENUM_ATTRIBUTES {
  ...
  llb_buildsystem_command_status_kind_is_scanning BUILRA_SWIFT_NAME(isScanning) = 0,

  /// Indicates the command is up-to-date, and does not need to run.
  llb_buildsystem_command_status_kind_is_up_to_date BUILRA_SWIFT_NAME(isUpToDate) = 1,
  ...
  llb_buildsystem_command_status_kind_is_complete BUILRA_SWIFT_NAME(isComplete) = 2,
} llb_buildsystem_command_status_kind_t BUILRA_SWIFT_NAME(CommandStatusKind);

/// Cycle actions.
typedef enum BUILRA_ENUM_ATTRIBUTES {
  /// Indicates a rule will be forced to build.
  llb_cycle_action_force_build BUILRA_SWIFT_NAME(forceBuild) = 0,

  /// Indicates a rule's prior value will be supplied to a downstream rule.
  llb_cycle_action_supply_prior_value BUILRA_SWIFT_NAME(supplyPriorValue) = 1,
} llb_cycle_action_t BUILRA_SWIFT_NAME(CycleAction);

/// Scheduler algorithms
typedef enum BUILRA_ENUM_ATTRIBUTES {
  /// Command name priority queue based scheduling [default]
  llb_scheduler_algorithm_command_name_priority BUILRA_SWIFT_NAME(commandNamePriority) = 0,

  /// First in, first out
  llb_scheduler_algorithm_fifo = 1
} llb_scheduler_algorithm_t BUILRA_SWIFT_NAME(SchedulerAlgorithm);
...
typedef enum BUILRA_ENUM_ATTRIBUTES {
    ...
    llb_quality_of_service_user_initiated BUILRA_SWIFT_NAME(userInitiated) = 1,
...
} llb_quality_of_service_t BUILRA_SWIFT_NAME(QualityOfService);

/// Rule run reasons.
typedef enum BUILRA_ENUM_ATTRIBUTES {
    /// The rule has never been built.
    llb_rule_run_reason_never_built BUILRA_SWIFT_NAME(neverBuilt) = 0,

    /// The rule's signature has changed.
    llb_rule_run_reason_signature_changed BUILRA_SWIFT_NAME(signatureChanged) = 1,

    /// The rule has an invalid value.
    llb_rule_run_reason_invalid_value BUILRA_SWIFT_NAME(invalidValue) = 2,

    /// An input of the rule has been rebuilt.
    llb_rule_run_reason_input_rebuilt BUILRA_SWIFT_NAME(inputRebuilt) = 3,

    /// The rule has never been built.
    llb_rule_run_reason_forced BUILRA_SWIFT_NAME(forced) = 4,
} llb_rule_run_reason_t BUILRA_SWIFT_NAME(RuleRunReason);
...
BUILRA_EXPORT llb_buildsystem_t*
...
BUILRA_EXPORT llb_buildsystem_tool_t*
...
typedef struct llb_buildsystem_spawn_delegate_t_ {
  ...
  void (*process_started)(void* context, builra_pid_t pid);
...
BUILRA_EXPORT llb_buildsystem_command_t*
...
BUILRA_EXPORT llb_build_value_file_info_t
...
BUILRA_EXPORT llb_quality_of_service_t
